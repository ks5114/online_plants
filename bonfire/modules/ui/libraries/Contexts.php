...
class Contexts
{
    ...
    protected static $templateContextNav  = "<div class='m-menu__submenu '><span class='m-menu__arrow'></span><ul class='{class}'{extra}>\n{menu}</ul></div>\n";
    protected static $templateContextMenu = "<li class='{parent_class} {start} {open}' aria-haspopup='true' m-menu-submenu-toggle='hover'><a href='{url}' id='{id}' class='{current_class} '><i class='m-menu__link-icon {icon}'></i><span class='m-menu__link-text'>{text}</span><span class='m-menu__ver-arrow la la-angle-right '></span></a>{content}</li>\n";
    protected static $templateMenu        = "<li class='m-menu__item {start} {act} {sub_class}' aria-haspopup='true'><a {extra} href='{url}' title='{title}' class='m-menu__link m-menu__toggle  '><i class='m-menu__link-bullet m-menu__link-bullet--dot'><span></span></i><span class='m-menu__link-text'>{display}</span></a>\n</li>\n";
    protected static $templateSubMenu     = "<li class='{submenu_class} {act} ' aria-haspopup='true'><a href='{url}' {tgl}> <i class='m-menu__link-bullet m-menu__link-bullet--dot'><span></span></i><span class='m-menu__link-text'>{display}</span><i class='m-menu__ver-arrow la la-angle-right'></i></a><div class='m-menu__submenu '><span class='m-menu__arrow'></span><ul class='{child_class}'>{view}</ul></div></li>\n";

    protected static $templateContextEnd             = "";
    protected static $templateContextImage           = "<img src='{image}' alt='{title}' />";
    protected static $templateContextText            = "{title}";
    protected static $templateContextMenuAnchorClass = 'm-menu__link m-menu__toggle ';
    protected static $templateContextMenuDashboardClass = 'm-menu__link ';
    protected static $templateContextMenuExtra       = "m-menu__link m-menu__toggle ";
    protected static $templateContextNavMobileClass  = 'mobile_nav';

    /** @var string The class name to attach to the outer ul tag. */
    protected static $outer_class = 'm-menu__nav  m-menu__nav--dropdown-submenu-arrow ';

    /** @var string The class to attach to li tags with children. */
    protected static $parent_class = 'm-menu__item m-menu__item--submenu';

    /** @var string The class to apply to li tags within ul tags inside. */
    protected static $submenu_class = 'm-menu__item ';

    protected static $menu_class = 'm-menu__item start';

    /** @var string The class to apply to ul tags within li tags. */
    protected static $child_class = 'm-menu__subnav';
    ...
    public static function render_menu($mode = 'text', $order_by = 'normal', $top_level_only = false, $benchmark = false)
    {
        ...
        foreach ($contexts as $context) {
            ...
            if (self::$ci->auth->has_permission($viewPermission)
                ...
                $dashboardView = $context == 'dashboard' ? self::$templateContextMenuDashboardClass : self::$templateContextMenuAnchorClass;
                $menu .= str_replace(
                    array('{open}','{icon}','{start}','{parent_class}', '{url}', '{id}', '{current_class}', '{title}', '{extra}', '{text}', '{content}'),
                    array(
                        self::$ci->uri->segment(2) == $context ? 'm-menu__item--open' : '',
                        check_icon($context),
     ...
     */
    public static function context_nav($context = null, $class = 'm-menu__link m-menu__toggle  ', $ignore_ul = false)
    {
    ...
    {
        $search = array('{act}','{tgl}', '{submenu_class}','{url}', '{display}', '{child_class}', '{view}');
        $list   = '';
        //testdata(self::$menu);
        foreach (self::$menu as $topic_name => $topic) {
            if (count($topic) <= 1) {
                ...
                $list .= str_replace(
                    $search,
                    array(
                        $module == self::$ci->uri->segment(3) ? "m-menu__item--active m-menu__item--open" : "",
       ...
       // testdata(array(self::$child_class, '', $list));
        return str_replace(
            array('{class}', '{extra}', '{menu}'),
    ...
    private static function buildItem($module, $title, $display_name, $context, $menu_view = '')
    {
        ...
        if (empty($menu_view)) {
            $role_id = self::$ci->session->userdata('role_id');
            $sub_class = (($role_id==2 && $module=='users') ? 'hide':'');
            return str_replace(
                array('{start}','{act}','{extra}', '{url}', '{title}', '{display}','{sub_class}'),
                array(
                    self::$ci->uri->segment(2) == 'dashboard' ? 'start' : '',
                    $module == self::$ci->uri->segment(3) ? 'm-menu__item--active' : '',
                    $module == self::$ci->uri->segment(3) ? 'class="m-menu__link m-menu__toggle  " ' : '',
        ...
        return str_replace(
            array('{act}','{tgl}','{opn}','{submenu_class}', '{url}', '{display}', '{child_class}', '{view}'),
            array(
                $module == self::$ci->uri->segment(3) ? "m-menu__item--active m-menu__item--open" : "",
                "class='m-menu__link m-menu__toggle'",
                $module == self::$ci->uri->segment(3) ? "m-menu__item--open" : "",
                self::$submenu_class,
                'javascript:void(0);',
                $displayName,
                self::$child_class,
